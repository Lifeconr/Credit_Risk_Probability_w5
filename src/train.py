...
def load_data():
    ...
    try:
        ...
        # Define expected target column names based on your pipeline output
        possible_target_cols = ['target__is_high_risk', 'is_high_risk']
        target_col = None
        ...
        logging.info(f"Loaded data with {X.shape[0]} rows and {X.shape[1]} features. Target column: '{target_col}'.")
        return X, y
...
def train_models():
    ...
    try:
        ...
        logging.info(f"Data split into {X_train.shape[0]} training and {X_test.shape[0]} testing samples.")
        ...
        logging.info("Starting to iterate through models_to_train.")
        ...
        for model_name, config in models_to_train.items():
            with mlflow.start_run(run_name=model_name) as run:
                ...
                grid_search = GridSearchCV(
                    ...
                    cv=5,
                    scoring='roc_auc',
                    n_jobs=1,  # Keeping n_jobs=1 for stability
                    verbose=1
                )
                
                logging.info(f"Starting GridSearchCV fit for {model_name}...")
        ...
        if best_model_overall:
            ...
            with mlflow.start_run(run_name="Register_Best_Overall_Model") as reg_run:
                ...
                logging.info(f"Best overall model '{best_model_overall['name']}' saved locally "
                            f"and registered as 'CreditRiskClassifier' in MLflow Model Registry.")
                logging.info(f"Best overall model ROC AUC: {best_roc_auc_score:.4f}")
        else:
            logging.warning("No models were trained or evaluated successfully.")
            
    except Exception as e:
        logging.error(f"An error occurred during the model training process: {str(e)}")
        import traceback
        traceback.print_exc()
        raise
